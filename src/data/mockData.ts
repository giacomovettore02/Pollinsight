...
export interface VarroaDetection {
  ...
  miteCount: number;
  images: string[]; // URLs to detection images
  severity: 'none' | 'low' | 'medium' | 'high';
  notes?: string;
}

// Generate mock varroa detection history for a hive
export function generateVarroaHistory(_hiveId: string, hasActiveVarroa: boolean): VarroaDetection[] {
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    // Random detections based on whether hive has active varroa
    const dayOfMonth = date.getDate();

    if (hasActiveVarroa) {
      // More frequent detections for hives with varroa
      if (i < 7 && Math.random() > 0.3) {
        // Recent days - higher chance of detection
        history.push({
          date: dateStr,
          detected: true,
          ...
          images: [
            ...
            'https://images.pexels.com/photos/47356/bumble-bee-bee-insect-insect-47356.jpeg?auto=compress&cs=tinysrgb&w=400',
          ],
          severity: 'medium',
      ...
      // Healthy hive - occasional false positives or clean readings
      if (dayOfMonth === 15 && Math.random() > 0.7) {
        history.push({
          date: dateStr,
          detected: true,
          confidence: 55,
          ...
          images: [],
          severity: 'low',
          notes: 'Falso positivo verificato. Nessuna azione richiesta.',
        });
      } else {
        history.push({
          date: dateStr,
...
// Pre-generate varroa history for all hives
export const varroaHistory: Record<string, VarroaDetection[]> = {
  'hive-1': generateVarroaHistory('hive-1', false),
  'hive-2': generateVarroaHistory('hive-2', true),
  'hive-3': generateVarroaHistory('hive-3', false),
  'hive-4': generateVarroaHistory('hive-4', false),
  'hive-5': generateVarroaHistory('hive-5', true),
  'hive-6': generateVarroaHistory('hive-6', false),
};
